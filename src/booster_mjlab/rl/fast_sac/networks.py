...
Actor (tanh-squashed Gaussian policy) and distributional critic (C51) networks
ported from holosoma's FastSAC implementation.
"""
...
class Actor(nn.Module):
    ...
    def output_std(self) -> torch.Tensor:
        ...
        The runner logger calls ``self.alg.get_policy().output_std``.  For SAC
        the noise is state-dependent, so we just return a reasonable proxy
        (the midpoint of the allowed log-std range exponentiated).
        """
...
class Critic(nn.Module):
    ...
    def forward(self, obs: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        """Returns stacked logits of shape ``(num_q_networks, batch, num_atoms)``."""
    ...
    def projection(
        ...
        """Returns stacked projected distributions ``(num_q_networks, batch, num_atoms)``."""
        device = self.q_support.device
        return torch.stack(
            [
                qnet.projection(
                    obs, actions, rewards, bootstrap, discount, self.q_support, device
                )
    ...
    def get_value(self, probs: torch.Tensor) -> torch.Tensor:
        """Compute expected value from probability distribution over support."""
