...
from booster_mjlab.amp.modules.amp import AMP, construct_amp
from booster_mjlab.rl.fast_sac.networks import Actor, Critic
...
class FastSAC:
    ...
    def __init__(
        ...
        *,
        ...
        self.amp = amp
        self.discriminator = self.amp.discriminator if self.amp is not None else None
    ...
    # ------------------------------------------------------------------
    # Automatic Mixed Precision (AMP) context helper
    # ------------------------------------------------------------------
    @contextmanager
    def _maybe_amp(self):
    ...
    # ------------------------------------------------------------------
    # PPO-compatible interface used by OnPolicyRunner
    # ------------------------------------------------------------------

    def act(self, obs: TensorDict) -> torch.Tensor:
        """Select actions for all envs.  Called by the runner's collect loop."""
        # Flatten TensorDict to actor/critic tensors using configured obs groups.
    ...
    def predict_style_reward(self, amp_history: torch.Tensor) -> torch.Tensor:
        if self.amp is None:
            return torch.zeros(
                amp_history.shape[0],
                device=amp_history.device,
                dtype=amp_history.dtype,
            )
    ...
    def process_env_step(
        ...
        # If true final observations are unavailable on timeouts, disable timeout
        # bootstrapping to avoid using post-reset observations as next state.
        has_final_obs = False
            ...
            if isinstance(final_obs, dict):
                ...
                if actor_keys and critic_keys:
                    final_actor_obs = torch.cat(
                        [final_obs[k] for k in self._actor_obs_group_names], dim=-1
                    ).to(flat_next_actor_obs.device)
                    final_critic_obs = torch.cat(
                        [final_obs[k] for k in self._critic_obs_group_names], dim=-1
    ...
    def update(self) -> dict[str, float]:
        ...
        with torch.no_grad():
            data["observations"] = self._normalize_obs_fn(
        ...
        amp_updates = (
            ...
            and self.discriminator_optimizer is not None
            and self.amp.has_current_policy_samples()
        )
        if amp_updates:
            amp_policy_generator, amp_expert_generator = self.amp.generators(
                num_mini_batch=self.num_updates,
                mini_batch_size=samples_per_update,
        ...
        total_accuracy_policy_num = 0.0
        total_accuracy_expert_num = 0.0
        total_accuracy_policy_den = 0.0
        total_accuracy_expert_den = 0.0
        ...
        for i in range(self.num_updates):
            ...
            if amp_updates:
                assert amp_policy_generator is not None
                assert amp_expert_generator is not None
                ...
                amp_loss, grad_pen_loss, amp_metrics = (
                    self.amp.compute_discriminator_batch(
                        sample_amp_policy,
                        sample_amp_expert,
                        grad_penalty_lambda=self.amp_grad_penalty_lambda,
                    )
                )
                ...
                total_amp_loss += amp_loss.detach().item()
                total_grad_pen_loss += grad_pen_loss.detach().item()
                total_policy_pred += amp_metrics["policy_pred"].item()
                total_expert_pred += amp_metrics["expert_pred"].item()
                total_accuracy_policy_num += amp_metrics["accuracy_policy_num"].item()
                total_accuracy_expert_num += amp_metrics["accuracy_expert_num"].item()
                total_accuracy_policy_den += amp_metrics["accuracy_policy_den"].item()
                total_accuracy_expert_den += amp_metrics["accuracy_expert_den"].item()
        ...
        if amp_updates:
            loss_dict["Discriminator/loss"] = total_amp_loss / n
            loss_dict["Discriminator/grad_penalty"] = total_grad_pen_loss / n
            loss_dict["Discriminator/policy_pred"] = total_policy_pred / n
            loss_dict["Discriminator/expert_pred"] = total_expert_pred / n
            loss_dict["Discriminator/accuracy_policy"] = (
                total_accuracy_policy_num / max(1.0, total_accuracy_policy_den)
            )
            loss_dict["Discriminator/accuracy_expert"] = (
                total_accuracy_expert_num / max(1.0, total_accuracy_expert_den)
            )
    ...
    def _update_critic(
        ...
        with self._maybe_amp():
            ...
            with torch.no_grad():
                ...
                modified_rewards = (
                    rewards
                    - discount * bootstrap * self.log_alpha.exp() * next_log_probs
                )

                target_distributions = self.critic_target.projection(
                    next_critic_obs, next_actions, modified_rewards, bootstrap, discount
                )
    ...
    # ------------------------------------------------------------------
    # Remaining PPO-compatible interface
    # ------------------------------------------------------------------
    ...
    def train_mode(self) -> None:
        ...
        if self._compile:
            ...
            self._normalize_critic_obs_fn = torch.compile(
                self.critic_obs_normalizer.forward
            )
    ...
    def save(self) -> dict:
        ...
        if self.discriminator_optimizer is not None:
            saved_dict["discriminator_optimizer_state_dict"] = (
        ...
        if load_cfg.get("critic"):
            ...
            if "critic_obs_normalizer_state_dict" in loaded_dict:
                self.critic_obs_normalizer.load_state_dict(
                    loaded_dict["critic_obs_normalizer_state_dict"], strict=strict
                )
    ...
    def broadcast_parameters(self) -> None:
        ...
        This method is invoked by the runner in distributed training scenarios.
        Distributed training is currently not supported for FastSAC, so calling
        this method indicates a misconfiguration and will raise an error.
        """
    ...
    # ------------------------------------------------------------------
    # Construction from config (called by the runner)
    # ------------------------------------------------------------------
    ...
    def construct_algorithm(
        ...
        """Construct the FastSAC algorithm from a config dict.

        This mirrors ``PPO.construct_algorithm`` so that ``OnPolicyRunner``
        ...
        if "amp" in cfg.get("obs_groups", {}) or "amp" in obs.keys():
        ...
        if obs_normalization:
            obs_norm: nn.Module = EmpiricalNormalization(
                shape=n_actor_obs, device=device
            )
        ...
        has_amp_group = "amp" in cfg["obs_groups"] and len(cfg["obs_groups"]["amp"]) > 0
        has_amp_cfg = (
            cfg.get("dataset_root") is not None and cfg.get("discriminator") is not None
        )
        if has_amp_group and not has_amp_cfg:
            raise ValueError(
                "FastSAC AMP requires `dataset_root` and `discriminator` in runner config."
            )
        if has_amp_cfg and not has_amp_group:
            raise ValueError(
                "FastSAC AMP requires an 'amp' observation group in `obs_groups`."
            )
        amp = None
        if has_amp_group:
        ...
        alg = FastSAC(
            ...
            num_steps=num_steps,
            amp_discriminator_lr=alg_cfg.pop("amp_discriminator_lr", 1e-3),
            amp_grad_penalty_lambda=alg_cfg.pop("amp_grad_penalty_lambda", 10.0),
            device=device,
        )

        # Store observation group names to flatten TensorDicts during collection.
